...
void *ui_initialize(void *_self, UIOptions _options, int32_t *error_code) {
    ...
    WNDCLASSEX uiwndclass = {
        ...
        0, // cbClsExtra: Number of extra bytes to allocate following the
           // window-class structure
        0, // cbWndExtra: The number of extra bytes to allocate following the
           // window instance.
        GetModuleHandle(0), // hInstance: A handle to the instance that contains
        ...
        NULL, // lpszMenuName: Pointer to a null-terminated character string
              // that specifies the resource name of the class menu
        ui_winclass, // lpszClassName: A pointer to a null-terminated string or
                     // is an atom.
        NULL // hIconSm: A handle to a small icon that is associated with the
             // window class.
    };
    ...
    if (RegisterClassEx(&uiwndclass)) {
        ...
        window = CreateWindowEx(
            0, // dwExStyle: The extended window style of the window being
               // created.
            ...
            NULL, // hWndParent:  handle to the parent or owner window of the
                  // window being created.
            NULL, // hMenu: A handle to a menu, or specifies a child-window
                  // identifier, depending on the window style.
            GetModuleHandle(0), // hInstance: A handle to the instance of the
